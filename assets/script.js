...
moonGroup.position.set(16, 24, -26);
scene.add(moonGroup);
...
const moon = new THREE.Mesh(
  new THREE.SphereGeometry(moonRadius, isMobile ? 32 : 48, isMobile ? 24 : 36),
  moonMaterial,
);
moonGroup.add(moon);

const moonLight = new THREE.PointLight(0xffe7b0, 1.8, 28);
moonLight.position.set(-3, 2, 4);
...
const craterDirections = [
  new THREE.Vector3(-0.48, 0.48, 0.74),
  new THREE.Vector3(0.28, 0.62, 0.73),
  new THREE.Vector3(0.58, -0.18, 0.79),
  new THREE.Vector3(-0.2, -0.48, 0.86),
  new THREE.Vector3(-0.62, -0.08, 0.76),
];

craterDirections.forEach((direction, index) => {
  direction.normalize();
  const crater = new THREE.Mesh(
    new THREE.SphereGeometry(0.6 + (index % 2) * 0.28, 16, 10),
    craterMaterial,
  );
  crater.position.copy(direction).multiplyScalar(moonRadius * 1.01);
  crater.scale.set(1, 1, 0.18);
...
const wishList = [
  ...
  {
    text: "Quỳnh đăng ảnh nhiều hơn một chút nhé, để những ngày của anh cũng có thêm lý do để mỉm cười.",
    img: "./assets/2.jpg",
  },
  {
    text: "Em nhớ đăng ảnh thường xuyên nhé, để anh còn được ngắm cô gái xinh đẹp và đáng yêu nhất trong mắt anh.",
    img: "./assets/3.jpg",
  },
  {
    text: "Mỗi lần Quỳnh đăng ảnh là một lần newsfeed của anh bỗng trở nên vui vẻ và dịu dàng hơn hẳn.",
    img: "./assets/1.jpg",
  },
  {
    text: "Quỳnh cứ xinh đẹp như thế này thì đăng ảnh nhiều lên nhé, anh ngắm một chút rồi lại có thêm động lực làm việc.",
    img: "./assets/2.jpg",
  },
  {
    text: "Anh không cần điều gì quá lớn lao, chỉ cần thỉnh thoảng được thấy nụ cười của Quỳnh qua những tấm ảnh là đã vui rồi.",
    img: "./assets/3.jpg",
  },
  {
...
function onPointerDown(event) {
  if (!event.target.closest(".top-bar")) startAudio();
...
function animate() {
  ...
  moonGroup.rotation.y = time * 0.015;
